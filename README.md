...
src/
  ...
  components/brand-carousel.css Narrow logo tiles, motion and manual-scroll states
...
- HashImagin is the personal project from the resume. The second Work card is Multi-Agentic System, described by the owner as a social-media brand-collaboration platform that curates leads and supports outreach to growth/marketing specialists. No project date, employer, repository, deployment, or performance figures are invented. The agentic log analyser remains in the Cisco experience entry.
...
Content-creation checks verify placement immediately after mentorship, community wording, updated work/highlight content, both social destinations, eight loaded logos and the approved Gamma text tile, narrow-strip geometry, automatic motion, seamless loop widths, pause/resume, keyboard/manual scrolling, reduced motion, and failed-image fallbacks. Checks cover both themes, mobile/desktop layout, print exclusion, and accessibility. Navigation tests cover six destinations and the compact-menu breakpoint; expanded navigation uses document flow rather than a fixed spacer. Keyboard-driven popup tests stub the third-party responses to verify LinkedIn and X navigation without relying on external availability or authentication. The section remains functional when the 3D download fails.
